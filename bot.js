...
const { smsg, sleep, getBuffer } = require('./System/lib/myfunction');
const { imageToWebp, videoToWebp, writeExifImg, writeExifVid, addExif } = require('./System/lib/exif')
...
const clientstart = async (socket = null, pNumber = null) => {
    if (socket) globalSocket = socket;
    const sock = socket || globalSocket;

    if (globalClient) {
        try { globalClient.end(); } catch (e) { }
        globalClient = null;
    }
    if (pNumber) {
        ...
        if (fs.existsSync(sessionDir)) {
            ...
            console.log("Session cleared for new pairing");
        }
    }
    ...
    if (config().status.terminal && !client.authState.creds.registered) {
        ...
        if (!phoneNumber) {
            if (sock) {
                sock.emit('status', 'offline');
                // Don't block with question, just wait for user to send number via UI
                return;
            }
            phoneNumber = await question('/> please enter your WhatsApp number, starting with 62:\n> number: ');
        }

        try {
            if (sock) {
                console.log(`Waiting for connection to stabilize before requesting code...`);
                sock.emit('log', 'Waiting 6 seconds for connection stabilization...');
                await delay(6000);
            }
            ...
            if (sock) sock.emit('pairing-code', code);
        } catch (e) {
            console.error('Pairing error details:', e);
            if (sock) sock.emit('log', `Error pairing: ${e.message}`);
        }
    }
    ...
    client.ev.on('connection.update', (update) => {
        ...
        // Use globalSocket to ensure status is emitted even after restart
        if (globalSocket) {
            if (connection === 'open') {
                globalSocket.emit('status', 'online');
                console.log('[Socket.IO] Emitted status: online');
            }
            else if (connection === 'close') {
                globalSocket.emit('status', 'offline');
                console.log('[Socket.IO] Emitted status: offline');
            }
            else if (connection === 'connecting') {
                globalSocket.emit('status', 'pairing');
                console.log('[Socket.IO] Emitted status: pairing');
            }
        }
        const { konek } = require('./System/lib/connection/connect')
    ...
    client.albumMessage = async (jid, array, quoted) => {
        ...
        for (let content of array) {
            ...
            img.message.forwardedNewsletterMessageInfo = {
                ...
                serverMessageId: 1,
                newsletterName: `INFORMATION - TECHNIFY DIGITAL`,
                contentType: 1,
                timestamp: new Date().toISOString(),
                senderName: "TECHNIFY DIGITAL",
...
const reset = () => {
    ...
    if (fs.existsSync(sessionDir)) {
        fs.rmSync(sessionDir, { recursive: true, force: true });
        console.log("Session cleared via reset");
    }
    if (globalSocket) {
        globalSocket.emit('status', 'offline');
        globalSocket.emit('log', 'System reset. Ready for new pairing.');
    }
    clientstart();
}
...
// clientstart()
module.exports = { start: clientstart, reset: reset, getSession }
